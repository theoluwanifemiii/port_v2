...
function shell({ eyebrow, headline, subline, body, ctaHref, ctaLabel, footerNote, }) {
    return `<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="UTF-8"/>
...
function qaFields(questionnaire) {
...
// ─── Email templates ──────────────────────────────────────────────────────────
function questionnaireOwnerEmail(name, email, company, projectType, questionnaire) {
    const body = `
    ${infoField("From", name)}
...
function questionnaireClientEmail(name, projectType, questionnaire) {
    const body = `
...
function contactOwnerEmail(name, email, company, project, budget) {
    const body = `
    ${infoField("From", name)}
    ${infoField("Email", email, true)}
    ${company ? infoField("Company", company) : ""}
...
export const sendEmail = httpAction(async (_ctx, request) => {
    ...
    try {
        ...
        const isQuestionnaire = typeof projectType === "string" && Array.isArray(questionnaire);
        if (isQuestionnaire) {
            // Notify Olu
            await sendViaResend({
